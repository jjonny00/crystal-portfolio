...
// Now properly targets project anchors with fresh position updates
...
const UnifiedCameraController = ({ 
  ...
  const findAnchorInFacet = (facetKey) => {
    if (!facetRefs || !facetRefs.current) {
      console.warn('⚠️ No facet refs available for anchor search');
      return null;
    }
    ...
    if (facetIndex === -1) {
      console.warn(`⚠️ Unknown facet key: ${facetKey}`);
      return null;
    }
    
    const facetRef = facetRefs.current[facetIndex];
    if (!facetRef || !facetRef.current) {
      console.warn(`⚠️ Facet ref not available for ${facetKey} (index ${facetIndex})`);
      return null;
    }
  ...
  const getCameraTarget = (cameraConfig, focusedFacet, cameraState) => {
    ...
    if (cameraState === 'project' && focusedFacet && facetRefs) {
      ...
      if (anchorPosition) {
        ...
        console.warn(`⚠️ No anchor found for ${focusedFacet}, using default target`);
        return {
          ...cameraConfig,
  ...
  useEffect(() => {
    ...
    if (configChanged) {
      ...
      if (animationData.state === 'hero' && positionDistance > 3) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`📹 Project focus camera update: ${focusedFacet}, distance: ${positionDistance.toFixed(2)}, using anchor: ${enhancedConfig.description?.includes('anchor')}`);
        }
      } else {
