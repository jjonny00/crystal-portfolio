// src/components/ui/ProjectDetailCard.jsx - Mobile-optimized version
import React, { useState, useEffect } from 'react';
import { animated, useSpring } from '@react-spring/web';

/**
 * Mobile-optimized project detail card with hidden image and compact layout
 */
const ProjectDetailCard = ({ project, visible, onClose, isMobile = false }) => {
  ...
  return (
    <animated.div 
      style={{
        ...springProps,
        ...getMobileStyles(),
        borderRadius: isMobile ? '12px' : '16px', // Slightly smaller radius on mobile
        background: 'rgba(25, 25, 30, 0.9)', // Slightly more opaque on mobile
    ...
    >
      ...
      {/* Header section with project title - more compact on mobile */}
      <div style={{ 
        display: 'flex', 
        alignItems: 'center', 
        gap: isMobile ? '12px' : '16px',
        paddingRight: isMobile ? '32px' : '40px' // Account for close button
      }}>
        <animated.div 
          style={{
            ...titleSpring,
            backgroundColor: projectColor,
            width: isMobile ? '40px' : '48px',  // Smaller icon on mobile
        ...
        <animated.div style={{...titleSpring, flex: 1, minWidth: 0}}>
          <h2 style={{ 
            margin: 0, 
            fontSize: isMobile ? '18px' : '24px', // Smaller title on mobile
            fontWeight: '600',
      ...
      {/* Technologies section - more compact on mobile */}
      <animated.div 
        style={{
          ...contentSpring,
          display: 'flex',
      ...
      >
        {project.technologies && project.technologies.map((tech, index) => (
          <span key={index} style={{
            ...
            color: 'white',
            padding: isMobile ? '3px 8px' : '4px 10px', // Smaller padding on mobile
            borderRadius: isMobile ? '12px' : '16px',
            fontSize: isMobile ? '11px' : '12px', // Smaller text on mobile
            fontWeight: '500'
          }}>
            {tech}
          </span>
      ...
      {/* Description section - more compact on mobile */}
      <animated.div 
        style={{
          ...contentSpring,
          fontSize: isMobile ? '13px' : '14px', // Smaller text on mobile
          lineHeight: '1.5', // Tighter line height on mobile
          color: 'rgba(255, 255, 255, 0.9)',
          // Limit height on mobile to prevent taking up too much space
          ...(isMobile && {
            maxHeight: '60px',
            overflow: 'hidden',
            display: '-webkit-box',
      ...
      {/* PROJECT IMAGE HIDDEN FOR MOBILE - Only show on desktop */}
      ...
      {/* Links section - more compact on mobile */}
      <animated.div
        style={{
          ...contentSpring,
          display: 'flex',
        ...
        {project.demoUrl && (
          <a 
            ...
            style={{
              ...
              textDecoration: 'none',
              transition: 'all 0.2s ease',
              flex: isMobile ? '1' : 'none', // Expand to fill space on mobile
              justifyContent: 'center',
