...
// Time constants (not per-frame factors) so the follow feels identical whether
// the scene is running at 120fps or has dropped to 30. Long on purpose: the peak
...
const VerticalEnergyLine = () => {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      influence += (wantsPointer - influence) * (1 - Math.exp(-delta / POINTER_HANDOFF_MS));
      ...
      else pointerCurrent += (pointerTarget - pointerCurrent) * (1 - Math.exp(-delta / POINTER_FOLLOW_MS));
