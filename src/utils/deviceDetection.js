...
const classifyEnhancedPerformance = (capabilities, gpuInfo) => {
  ...
  if (isHighPerformanceDevice) {
    if (process.env.NODE_ENV === "development") console.log(`🚀 High-performance device detected: ${deviceModel}`);
    return 'high';
  }
  ...
  if (capabilities.isIPad && gpuInfo?.tier === 'apple-silicon') {
    if (process.env.NODE_ENV === "development") console.log('🍎 iPad with Apple Silicon detected - HIGH tier');
    return 'high';
  }
  
  // Recent iPads with good specs
  if (capabilities.isIPad && capabilities.cpuCores >= 6 && capabilities.estimatedRAM >= 4) {
    if (process.env.NODE_ENV === "development") console.log('🍎 High-spec iPad detected - HIGH tier');
    return 'high';
  }
...
export const detectDevice = () => {
  ...
  if (process.env.NODE_ENV === "development") console.log('🔍 Enhanced Device Detection Results:', {
...
export const enforcePortraitOnMobile = () => {
  if (isMobile() && screen.orientation && screen.orientation.lock) {
    try {
      ...
      if (process.env.NODE_ENV === "development") console.warn('Could not lock orientation:', e);
    }
  }
};
